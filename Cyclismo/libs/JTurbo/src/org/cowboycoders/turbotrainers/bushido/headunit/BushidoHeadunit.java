...
public class BushidoHeadunit extends AntTurboTrainer {
  ...
  public static final byte[] PACKET_RESET_ODOMETER = AntUtils.padToDataLength(new int[]{0xac,
      0x03, 0x01});

  public static final byte[] PACKET_DISCONNECT = AntUtils.padToDataLength(new int[]{0xac, 0x03});

  public static final byte[] PACKET_INIT_CONNECTION = AntUtils.padToDataLength(new int[]{0xac,
      0x03, 0x04});

  public static final byte[] PACKET_START_CYLING = AntUtils.padToDataLength(new int[]{0xac, 0x03,
      0x03});

  private static final long RESET_ODOMETER_TIMEOUT = TimeUnit.SECONDS.toNanos(5);

  private static final Byte[] PARTIAL_PACKET_CONNECTION_SUCCESSFUL = new Byte[]{(byte) 0xad,
      0x01, 0x04};
...
//   * Weak set 
//   */
//  private Set<TurboTrainerDataListener> dataChangeListeners = Collections.newSetFromMap(new
// WeakHashMap<TurboTrainerDataListener,Boolean>());
//  
  /**
   * Weak set
   */
  private final Set<BushidoButtonPressListener> buttonPressListeners = Collections.newSetFromMap
      (new WeakHashMap<BushidoButtonPressListener, Boolean>());
   ...
   * Call after start.
   */
  private <V extends ChannelMessage> void registerChannelRxListener(BroadcastListener<V>
      listener, Class<V> clazz) {
  ...
  private <V extends ChannelMessage> void unregisterRxListener(BroadcastListener<? extends
  ...
  public void startConnection() throws InterruptedException, TimeoutException {
    ...
    BushidoUpdatesListener updatesListener = new BushidoUpdatesListener(model, this
        .getMessageSender());
    BushidoBroadcastDataListener dataListener = new BushidoBroadcastDataListener(updatesListener);
    BushidoInternalButtonPressListener buttonListener = new BushidoInternalButtonPressListener
        (updatesListener);
  ...
  private void initConnection() throws InterruptedException, TimeoutException {
    ...
    MessageCondition condition = new MessageCondition() {
      ...
      public boolean test(StandardMessage msg) {
        ...
        if (!ArrayUtils.arrayStartsWith(PARTIAL_PACKET_CONNECTION_SUCCESSFUL, dataMessage.getData
            ()))
          return false;
        return true;
      }
  ...
  private void disconnect() throws InterruptedException, TimeoutException {
    ...
    MessageCondition condition = new MessageCondition() {
      ...
      public boolean test(StandardMessage msg) {
        if (!(msg instanceof DataMessage)) return false;
        DataMessage dataMessage = (DataMessage) msg;
        if (!ArrayUtils.arrayStartsWith(PARTIAL_PACKET_NO_CONNECTION, dataMessage.getData()))
          return false;
        return true;
      }

    };
    MessageCondition chainedCondition = MessageConditionFactory.newChainedCondition(AntUtils
        .CONDITION_CHANNEL_TX, condition);
    sendAndRetry(msg, chainedCondition, 20, 1L, TimeUnit.SECONDS);
  }

  private StandardMessage sendAndRetry(final ChannelMessage msg, final MessageCondition
      condition, final int maxRetries, final long timeoutPerRetry, final TimeUnit timeoutUnit)
  ...
  public void resetOdometer() throws InterruptedException, TimeoutException {
    ...
    synchronized (model) {
      while (model.getVirtualDistance() > 0.000001) {
        channel.sendAndWaitForMessage(msg, AntUtils.CONDITION_CHANNEL_TX, 10L, TimeUnit.SECONDS,
            null);
        long currentTimestamp = System.nanoTime();
