...
import com.google.android.apps.mytracks.content.CourseTrackPointsColumns;
import com.google.android.apps.mytracks.content.CourseTracksColumns;
import com.google.android.apps.mytracks.content.CourseWaypointsColumns;
...
import org.cowboycoders.cyclisimo.content.MyTracksCourseProvider;
 ...
 * @author Youtao Liu
 */
public class MyTracksCourseUtilsTest extends AndroidTestCase {
  ...
  private Factory orginalFactory;
  ...
  protected void setUp() throws Exception {
    ...
    MyTracksCourseProvider provider = new MyTracksCourseProvider();
    provider.attachInfo(context, null);
    mockContentResolver.addProvider(MyTracksProviderUtils.AUTHORITY, provider);
    setContext(context);
    this.orginalFactory = MyTracksProviderUtils.Factory.getInstance();
    MyTracksProviderUtils.Factory.overrideInstance(new MyTracksCourseProviderUtils.Factory());
    providerUtils = MyTracksProviderUtils.Factory.get(context);
    Factory f = new MyTracksCourseProviderUtils.Factory();
    providerUtils = f.newForContext(context);
  ...
  protected void tearDown() throws Exception {
    super.tearDown();
    MyTracksProviderUtils.Factory.overrideInstance(orginalFactory);
  }
  ...
  private List<Location> testIterator(long trackId, int numPoints, int batchSize,
    ...
    ((MyTracksCourseProviderUtils) providerUtils).setDefaultCursorBatchSize(batchSize);
  ...
  public void testCreateTrack() {
    ...
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseTracksColumns._ID))
        .andReturn(columnIndex++);
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseTracksColumns.NAME)).andReturn(
        columnIndex++);
  ...
  public void testDeleteAllTracks() {
    ...
    Cursor tracksCursor = contentResolver.query(CourseTracksColumns.CONTENT_URI, null, null, null,
        CourseTracksColumns._ID);
    assertEquals(1, tracksCursor.getCount());
    Cursor tracksPointsCursor = contentResolver.query(CourseTrackPointsColumns.CONTENT_URI, null, null,
        null, CourseTrackPointsColumns._ID);
    assertEquals(10, tracksPointsCursor.getCount());
    Cursor waypointCursor = contentResolver.query(CourseWaypointsColumns.CONTENT_URI, null, null,
        null, CourseWaypointsColumns._ID);
    assertEquals(1, waypointCursor.getCount());
    // Delete all.
    providerUtils.deleteAllTracks();
    // Check whether all have been deleted. 
    tracksCursor = contentResolver.query(CourseTracksColumns.CONTENT_URI, null, null, null,
        CourseTracksColumns._ID);
    assertEquals(0, tracksCursor.getCount());
    tracksPointsCursor = contentResolver.query(CourseTrackPointsColumns.CONTENT_URI, null, null,
        null, CourseTrackPointsColumns._ID);
    assertEquals(0, tracksPointsCursor.getCount());
    waypointCursor = contentResolver.query(CourseWaypointsColumns.CONTENT_URI, null, null,
        null, CourseWaypointsColumns._ID);
  ...
  public void testCreateContentValues_waypoint() {
    ...
    assertEquals(waypointId, contentValues.get(CourseWaypointsColumns._ID));
    assertEquals(22 * 1000000, contentValues.get(CourseWaypointsColumns.LONGITUDE));
    assertEquals(TEST_DESC, contentValues.get(CourseWaypointsColumns.DESCRIPTION));
    assertEquals(startTime, contentValues.get(CourseWaypointsColumns.STARTTIME));
    assertEquals(minGrade, contentValues.get(CourseWaypointsColumns.MINGRADE));
  }
  ...
  public void testCreateWaypoint() {
    ...
    int startColumnIndex = 1;
    int columnIndex = startColumnIndex;
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseWaypointsColumns._ID))
        .andReturn(columnIndex++);
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseWaypointsColumns.NAME)).andReturn(
        columnIndex++);
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseWaypointsColumns.TRACKID)).andReturn(
        columnIndex++);
    columnIndex = startColumnIndex;
    // Id
    AndroidMock.expect(cursorMock.isNull(columnIndex++)).andReturn(false);
  ...
  public void testDeleteWaypoint_onlyOneWayPoint() {
    ...
    DescriptionGenerator descriptionGenerator = new DescriptionGenerator() {
      ...
      public String generateWaypointDescription(TripStatistics tripStatistics) {
        return MyTracksCourseUtilsTest.MOCK_DESC;
      }
  ...
  public void testDeleteWaypoint_hasNextWayPoint() {
    ...
    assertNull(providerUtils.getWaypoint(1));
    assertEquals(MyTracksCourseUtilsTest.MOCK_DESC, providerUtils.getWaypoint(2)
        .getDescription());
  }
  ...
  public void testCreateTrackPoint() {
    ...
    int index = 1;
    // Id
    AndroidMock.expect(cursorMock.getColumnIndex(CourseTrackPointsColumns._ID)).andReturn(index++);
    // Longitude
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseTrackPointsColumns.LONGITUDE)).andReturn(
        index++);
    // Latitude
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseTrackPointsColumns.LATITUDE)).andReturn(
        index++);
    // Time
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseTrackPointsColumns.TIME))
        .andReturn(index++);
    // Speed
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseTrackPointsColumns.SPEED)).andReturn(
        index++);
    // Sensor
    AndroidMock.expect(cursorMock.getColumnIndexOrThrow(CourseTrackPointsColumns.SENSOR)).andReturn(
        index++);
   ...
   *          {@link MyTracksCourseUtilsTest#getTrack(long, int)}
